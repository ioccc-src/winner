...
This is FAQ version **30.00 2026-09-15**.
...
- **Q 1.7**: <a class="normal" href="#rule17">What are the details behind Rule 17 - use mkiocccentry?</a>
...
- **Q 1.13**: <a class="normal" href="#using_github">May I use GitHub or other services to develop my submission?</a>
...
- `all`
    * The `all` rule **MUST be the first make rule**, and it should compile your submission.
    The `all` rule **MUST be the default rule** when `make(1)` is invoked.
...
Although the `mkiocccentry(1)` tool only checks for those rules, the most up to date
`Makefile.example`
...
**IMPORANT**: Observe that the `all` rule is the first make rule so that `make all`,
and `make` both make everything for your submission.
...
**IMPORTANT NOTE**: Please read the [IOCCC markdown guidelines](markdown.html)
...
Please pay careful attention to [Rule 17 - Use mkiocccentry](next/rules.html#rule17-mkiocccentry) and in
...
of violating [Rule 17 - Use mkiocccentry](next/rules.html#rule17-mkiocccentry)!
...
<div id="rule17">
### Q 1.7: What are details behind Rule 17 - Use mkiocccentry?
</div>
...
conforms to [Rule 17 - Use mkiocccentry](next/rules.html#rule17-mkiocccentry).  Each of these tool has a `-h`
...
**IMPORTANT NOTE**: make **SURE** you have the most recent version of the
...
- `.auth.json` which contains information about the author or authors (that will only be looked at if the submission wins).
- `.info.json` which contains information about the submission.
...
**IMPORTANT NOTE**: Be sure that the `all` rule is the first make rule so `make all` is
equivalent to just typing `make`.
...
- `tarball` (double quoted string)
    ...
    **IMPORTANT:** this **MUST** match the actual tarball's **filename** in order for
    ...
    [Rule 17 - Use mkiocccentry](next/rules.html#rule17-mkiocccentry). The
...
**IMPORTANT NOTE**: some entries have html files **NOT** generated by a markdown file
