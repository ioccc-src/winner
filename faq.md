...
- **Q 10.6**: <a class="normal" href="#try_mastodon">What is Mastodon and why does IOCCC use it?</a>
...
The following are the details behind [Rule 17](next/rules.html#rule17).
...
conforms to [Rule 17](next/rules.html#rule17).
</p>
...
FAQ on "[including additional files](#extra-files)"
...
And yes, there is a 4 (UUID version 4) and the variants `0x8`, `0x9`, `0xa` and
`0xb` in there.
</p>
...
the
FAQ on "[submission Makefile](#submission_makefiles)"
and the
FAQ on "[Makefile compatibility](#make_compatibility)"
for help.
</p>
...
submission wins but it **MUST** still be valid according to `chkentry(1)`.
</p>

<p class="leftbar">
The `.info.json` file is created by `mkiocccentry(1)` and it holds information
about the submission itself. It also **MUST** be valid according to
`chkentry(1)`.
</p>
...
hints, comments, build and info files) **MUST** be <=
**28314624** octets (**27651K**) in size.
</p>

<p class="leftbar">
Your submission's xz compressed tarball **MUST** pass the `txzchk(1)` test. See
the [guidelines](next/guidelines.html) for more details on this tool.
</p>
...
submission's xz compressed tarball. See the [guidelines](next/guidelines.html) for
...
skipped. For more information, see the [guidelines](next/guidelines.html).
</p>
...
on your submission directory as well, _prior to_ forming the tarball.
</p>
...
17](next/rules.html#rule17)! In addition to `mkiocccentry(1)` creating and copying the
directories and files with these modes, it also verifies them after the fact and
if they are not correct it is an **error**.
</p>
...
things) for you and so will `chkentry(1)` (along with validating the JSON files
and running semantics checks on them). If these checks fail it is an **error**
and `mkiocccentry(1)` will fail. In this case (it fails and you used
...
FAQ on "[how to include additional files](#extra-files)"
...
`txzchk` only checks the submission tarball so you must verify these details
yourself.
</p>
...
FAQ on "[obtaining mkiocccentry](#obtaining_mkiocccentry)"
...
`make install` (as root or using `sudo(1)`) to help you run these tools from
...
<p class="leftbar">
These tools will link in the `jparse(3)` library and the `dbg` library and the
`dyn_array` library; `chkentry(1)` uses the JSON parser (`jparse`) to validate
and parse the [JSON](https://www.json.org/json-en.html) but the other tools use
parts of the `jparse` utility functions as well.
</p>
