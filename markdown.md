...
<span style="color:green">✔</span> Ok for all file types, eg. `.c`, `.h`, `.mk`, `.sh`, **EXCEPT** `.md` markdown files.
````<!---c-->
...
<span style="color:red">❌</span> Incorrect, missing empty line after closing fence (` ``` `).
````<!---markdown-->
```
int
main(int foo)
