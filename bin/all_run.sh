...
export USAGE="usage: $0 [-h] [-v level] [-V] [-d topdir] [-n] [-N]
	...
	tool_options	tool command line options to use before YYYY/dir
			NOTE: You must use -- after the tool command line option and before 'tool_options'
...
for YYYY in $(< "$TOP_FILE"); do
	...
	if [[ -z $DO_NOT_PROCESS ]]; then
	    ...
	    if [[ -z $NOOP ]]; then
		if [[ $V_FLAG -ge 5 ]]; then
		    echo "$0: debug[5]: about to run: $TOOL -d \"$TOPDIR\" -p \"$PANDOC_WRAPPER\"" \
			 "-P \"$PANDOC_WRAPPER_OPTSTR\" -u \"$REPO_URL\" -U \"$TOP_URL\" $TOOL_OPTIONS \"$WINNER_PATH\"" 1>&2
		fi
		eval "$TOOL" -d '"'"$TOPDIR"'"' -p '"'"$PANDOC_WRAPPER"'"' -P '"'"$PANDOC_WRAPPER_OPTSTR"'"' \
			-u '"'"$REPO_URL"'"' -U '"'"$TOP_URL"'"' "$TOOL_OPTIONS" '"'"$WINNER_PATH"'"'
		status="$?"
		if [[ $status -ne 0 ]]; then
		...
		echo "$0: debug[5]: because of -n, did not run: $TOOL -d \"$TOPDIR\" -p \"$PANDOC_WRAPPER\"" \
		     "-P \"$PANDOC_WRAPPER_OPTSTR\" -u \"$REPO_URL\" -U \"$TOP_URL\" $TOOL_OPTIONS \"$WINNER_PATH\"" 1>&2
	    fi
