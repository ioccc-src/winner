# IMPORTANT: Try the Makefile interface

The [top level Makefile](%%REPO_URL%%/Makefile) as a fair number of make rules that
drive these tools.  For an extensive list of these make rules, try:

``` <!---sh-->
    make help
```

Often, instead of invoking the [bin tolls](#bin-tools) directly,
using one of make rules is more effective, and more convenient.  For example:
...
One key action that [md2html.sh](%%REPO_URL%%/bin/md2html.sh) performs is
parameter substitution when constructing HTML files for the
[www.ioccc.org](https://www.iocc.org) website.  Tokens are intended to
match the "<code>%</code><code>%[A-Z0-9_]+%</code><code>%</code>" regular expression.  Tokens are found in the
markdown file and various [inc directory](%%REPO_URL%%/inc/index.html) files.
The token string is substituted for the value of the token in the HTML
file that is being formed.

Often the value of the token is determined by the
[md2html.cfg](index.html#md2html_cfg) configuration file.
There are other shell scripts that will make additional token
...
is special token has a special substitution process that is performed
...
tools in the [bin directory](#bin-tools) and exist only while the tool is running.
...
In all of the above HTML phase numbers, symbols of the form "<code>%%</code><code>TOKEN%%</code>" are
substituted.

Substitutions of the form "<code>%%</code><code>TOKEN%%</code>" are performed on included HTML content,
...
20-29, to not substitute all "<code>%%</code><code>TOKEN%%</code>"s.  For phases HTML 20-29, any
"<code>%%</code><code>TOKEN%%</code>" that is not substituted are passed thru without substitution.
...
All tokens (i.e., strings of the form "<code>%</code><code>%TOKEN%</code><code>%</code>") **MUST** be substituted (by
...
<div id="date-range">
#### Special DATE_RANGE token
</div>

The "<code>%</code><code>%DATE_RANGE%</code><code>%</code>" is a special token.

This "<code>%</code><code>%DATE_RANGE%</code><code>%</code>"
special token has a special substitution process that is performed
directly by the [md2html.sh](%%REPO_URL%%/bin/md2html.sh) script
outside of the above-mentioned parameter substitution process:
"<code>%</code><code>%DATE_RANGE%</code><code>%</code>".  The reason
that the "<code>%</code><code>%DATE_RANGE%</code><code>%</code>"
is handled separately from the other tokens is that the value of the
"<code>%</code><code>%DATE_RANGE%</code><code>%</code>" is dependent on
the directory on which the markdown source file is located.

For example, the **copyright date range** for the
[2001/dgbeards/index.html](../2001/dgbeards/index.html) HTML file
is obtained from the contents of the `.date-range` file in the same
directory, namely the [2001/dgbeards/.date-range](%%REPO_URL%%/2001/dgbeards/.date-range) file.

The purpose of the "<code>%</code><code>%DATE_RANGE%</code><code>%</code>"
token is to establish the **copyright date range**.  The token,
"<code>%</code><code>%DATE_RANGE%</code><code>%</code>", is replaced by
the value read from the `.date-range` file found in the same directory
as the markdown file being converted.  If no such `.date-range` file is
found, or if the `.date-range` file is not a readable non-empty file,
a default copyright date range is used.

Instead of having a single **copyright date range** for all constructed
HTML files, directory-specific **copyright date ranges** are used.  Thus,
when the calendar year changes, one does **NOT** need to rebuild the
entire website, including all of the compressed tarballs.

In all other cases besides the "<code>%</code><code>%DATE_RANGE%</code><code>%</code>" token,
any "<code>%</code><code>%[A-Z0-9_]+%</code><code>%</code>" tokens are replaced in the
HTML file that is being formed using the normal parameter substitution process.

**NOTE**: the "[copyright date range](#copyright-date-range)" section for more details
on "**copyright date ranges**'.
...
<div id="copyright-date-range">
## copyright date range
</div>

The content of the IOCCC is [Copyrighted and is under the CC BY-SA 4.0 License](#copyright).

The "**copyright date range**" for a given content represents a [copyright](#copyright) date range:
from the year in which the content was first created or obtained by the IOCCC,
through the most recent year in which that content was updated.

For HTML pages created by [bin tools](#bin-tools), a `.date-range` file located in
the same directory of the markdown source file is used to set the
"**copyright date range**" for generated content.

**NOTE**: the "[Special DATE_RANGE token](#date-range)" section for more details.

**NOTE**: see the
FAQ on "[use IOCCC content](../faq.html#licence)"
for more details on the IOCCC [copyright and license](#copyright).
