...
# usage
#
export USAGE="usage: $0 [-h] [-v level] [-V] [-d topdir] [-D docroot/] [-j jparse] [-n] [-N]
...
for YYYY in $(< "$TOP_FILE"); do
	...
	if [[ $V_FLAG -ge 3 ]]; then
	    echo "$0: debug[3]: about to run: $JPARSE_TOOL -q -- $ENTRY_JSON" 1>&2
	fi
	if "$JPARSE_TOOL" -q -- "$ENTRY_JSON"; then
	    if [[ $V_FLAG -ge 5 ]]; then
		echo "$0: debug[5]: valid JSON: $ENTRY_JSON" 1>&2
	    fi
	else
