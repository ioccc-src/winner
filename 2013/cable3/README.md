...
- Hercules graphics card with 720x348 2-color graphics (64KB video RAM), and CGA
...
The emulator uses the SDL graphics library for portability, and compiles for
Windows, macOS, Linux and probably most other 32-bit/64-bit systems too.
...
supplied is good for Mac OS X, Linux and probably other UNIXes. I have received
reports that the emulator works on Raspberry Pi/Android/ARM (you will need to
compile with -fsigned-char) and iOS. You will need to adjust the Makefile if
your system lacks sdl-config to correctly point to the SDL libraries and header
files.
...
`"keyboard driver"` KB which as it stands is suitable for UNIXes, but maybe not
...
PLEASE NOTE that under UNIXes the keyboard must be in raw mode for the emulator
...
image file called hd.img made using e.g. mkfile. Then use:
