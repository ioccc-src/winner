...
if [[ -z $NOOP ]]; then
    ...
    echo  "$0: debug[3]: -n disabled write: # author" 1>&2
    echo  "$0: debug[3]: -n disabled write:" 1>&2
fi
...
for author_handle in $AUTHOR_HANDLE_SET; do
    ...
    if [[ -z $NOOP ]]; then
	{
	    echo "[$FULL_NAME](/winners.html#$author_handle)"
	    echo "Location: [$LOCATION_CODE](/location.html#$LOCATION_CODE)"
	    echo
	} >> "$TMP_FILE"
    elif [[ $V_FLAG -ge 3 ]]; then
	echo  "$0: debug[3]: -n disabled write: [$FULL_NAME](/winners.html#$author_handle)" 1>&2
	echo  "$0: debug[3]: -n disabled write: Location: [$LOCATION_CODE](/location.html#$LOCATION_CODE)" 1>&2
	echo  "$0: debug[3]: -n disabled write:" 1>&2
    fi
done
