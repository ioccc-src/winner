#!/usr/bin/env bash
#
# fake.gen_inventory.sh - generate a fake .inventory.html file for an entry
#
...
# setup
#
export VERSION="1.0.0 2023-10-31"
...
# build the .inventory.html file
#
export INVENTORY_PATH="$FULL_WINNER_PATH/$URL_BASENAME"
...
<title>$YEAR $WINNER Inventory</title>
<meta name="description" content="IOCCC year: $YEAR IOCCC winner: $WINNER winner inventory">
<meta name="keywords" content="IOCCC,$YEAR,$WINNER,winner inventory">
</head>

<body>

<h1>$YEAR $WINNER Inventory</h1>

<p><ul>
<li><a href="$URL_PATH/index.html">index.html</a> - click here first</a></li>
<li>XXX - put link to prog source code - XXX</a></li>
<li><a href="$GITHUB_URL_PATH/Makefile">Makefile</a> - Makefile</a></li>
<li>XXX - put link to prog alt code if it exists - XXX</a></li>
<li>XXX - put link to prog original code - XXX</a></li>
<li>XXX - put links to other winners_rank numbered inventory files - XXX</a></li>
<li><a href="$GITHUB_URL_PATH/.inventory.html">.inventory.html</a> - full inventory of all winner files</a></li>
<li><a href="$GITHUB_URL_PATH/.gitignore">.gitignore</a> - list of files that should not be committed under git</a></li>
<li><a href="$GITHUB_URL_PATH/.id">.id</a> - winner id string - this file will be removed when .winner.json exists</a></li>
<li>XXX - put links to other null winners_rank inventory files - XXX</a></li>
</ul>
...
if ! cmp -s "$TMP_FILE" "$INVENTORY_PATH"; then
    ...
    #
    if [[ $V_FLAG -ge 1 ]]; then
	echo "$0: debug[1]: updating $INVENTORY_PATH"
    fi
    if [[ $V_FLAG -ge 1 ]]; then
	mv -f -v "$TMP_FILE" "$INVENTORY_PATH"
	status="$?"
    else
	mv -f "$TMP_FILE" "$INVENTORY_PATH"
	status="$?"
    fi
    if [[ $status -ne 0 ]]; then
	echo "$0: ERROR: mv -f -v $TMP_FILE $INVENTORY_PATH failed, error: $status" 1>&2
	exit 10
    fi
