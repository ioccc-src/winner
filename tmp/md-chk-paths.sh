#!/usr/bin/env bash
#
# md-chk-paths.sh - print local file paths referenced in markdown links
#
# For all markdown files, print markdown links that are local.
# We ignore URLs that start with "http:" or "https:".
# We ignore links that are internal to the file (i.e., "#name").
# We strip all "#name" from paths.
#
...
find . -name '*.md' ! -name markdown.md ! -path './tmp/*' -print |
	...
	sed -E \
		...
		echo "# Missing file linked in: $MD_PATH" 1>&2
		echo "# Markdown line: $MD_LINE" 1>&2
		echo "$ABS_LOCAL_FILE_PATH" 1>&2
	    fi
	 done
     done
