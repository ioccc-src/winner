...
\256. `gets()` is called as `m = gets(m)`) but trying to change it to use fgets()
proved more a problem. Since the input must come from the command line Cody
changed the buffer size to `ARG_MAX+1` which should be enough (again
