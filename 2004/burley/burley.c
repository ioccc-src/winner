...
jmp_buf p[4];
int poke(char *d){
return poke((char*)((srand(time((time_t*)&h)),
setjmp(p[2]),
...
longjmp(p[3],4)),1),
setjmp(p[0]),
i--&&!((g[1+i]^g[i])>>4)&&(longjmp(p[0],1),1),
setjmp(p[1]),
--j&&(!(k=(g[j]&15)-(g[j-1]&017)+1)||(!(j-1)&&(8==-k)))&&(longjmp(p[1],2),1),
s+=3*!j|!(++i^17),
(j*=i*=4)&&(i=1),
setjmp(p[3]),
5-j<2&&((j=0,
