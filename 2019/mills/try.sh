#!/usr/bin/env bash
#
# try.sh - demonstrate IOCCC winner 2019/mills
#
...
echo 1>&2
less -rEXF IOCCC-Rules-Guidelines.output.txt
echo 1>&2

echo "$ less IOCCC-hints.output.txt" 1>&2
read -r -n 1 -p "Press any key to continue (space = next page, q = quit): "
echo 1>&2
less -rEXF IOCCC-hints.output.txt
echo 1>&2
...
read -r -n 1 -p "Press any key to continue (space = next page, q = quit): "
echo 1>&2
less -rEXF Eugene_Onegin.output.txt
echo 1>&2
