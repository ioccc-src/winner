...
#
# NOTE: This needs a window at least 210 columns wide by at least 34 lines tall.
...
export USAGE="usage: $0 [-h] [-V] [seconds]
    ...
    [seconds]	seconds to run, 0 ==> unlimited (def: $CYCLE_LIMIT)
...
if [[ -z $CYCLE_LIMIT || $CYCLE_LIMIT -le 0 ]]; then
    CYCLE_LIMIT="600"
fi
...
while : ; do
    ...
    #
    for j in $(seq 0 3); do
	./prog "${TIMESTAMP:$j:1}" < prog.alt.c > "digit$j.out"
    done

    # display the time, blinking the colons every other second
    #
    if [[ -n $BLINK_ON ]]; then
	paste -d '' digit0.out digit1.out colon.out digit2.out digit3.out
	BLINK_ON=""
    else
	paste -d '' digit0.out digit1.out spaces.txt digit2.out digit3.out
	BLINK_ON="true"
    fi
